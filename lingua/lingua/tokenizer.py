...
class HuggingFaceTokenizer(Tokenizer):
    """Wraps a HuggingFace AutoTokenizer so lingua can use Qwen2 / LLaMA tokenizers."""
    ...
    def __init__(self, model_path: str) -> None:
        ...
        self.bos_id: int = (
            self.hf_tok.bos_token_id
            if self.hf_tok.bos_token_id is not None
            else self.hf_tok.cls_token_id or 0
        )
        self.eos_id: int = (
            self.hf_tok.eos_token_id
            if self.hf_tok.eos_token_id is not None
            else self.hf_tok.sep_token_id or 1
        )
        self.pad_id: int = self.hf_tok.pad_token_id if self.hf_tok.pad_token_id is not None else self.eos_id
        logger.info(
            f"Loaded HuggingFace tokenizer from {model_path}: "
            f"#words: {self.n_words} - BOS ID: {self.bos_id} - EOS ID: {self.eos_id}"
        )
