"""
Pass@128 evaluation for MTP (Multi-Token Prediction) models on GSM-Infinity test_small.

Loads a lingua MTP checkpoint, generates N completions per test example
using PackedCausalTransformerGenerator (MTP uses head 0 for generation),
and computes pass@k metrics.

Output format (metrics.jsonl) matches the dLLM/transformer eval scripts.

Usage:
    python -m apps.mtp.gsm_infinity.eval_pass128 \
