"""
Pass@128 evaluation for Mamba models on GSM-Infinity test_small.

Loads a lingua Mamba checkpoint, generates N completions per test example
using PackedCausalMambaGenerator, and computes pass@k metrics.

Output format (metrics.jsonl) matches the dLLM/transformer eval scripts.

Usage:
    python -m apps.mamba.gsm_infinity.eval_pass128 \
        --ckpt_dir lingua/saves/gsm_infinity/mamba_400M_gsm_.../checkpoints/0000005000 \
        --test_dir data/composition_hf/test_small \
        --n_samples 128 \
        --output_dir results/mamba_eval/mamba_400M_gsm_.../checkpoint-5000
"""
...
from apps.mamba.generate import PackedCausalMambaGenerator, PackedCausalMambaGeneratorArgs
...
def load_model(ckpt_dir: str):
    """Load and consolidate a lingua Mamba checkpoint."""
    ...
    model, tokenizer, train_cfg = load_consolidated_model_and_tokenizer(
        str(consolidate_path), model_cls=LMMamba, model_args_cls=LMMambaArgs,
    )
...
def evaluate(
    ...
    print(f"Loading Mamba model from {ckpt_dir}")
    ...
    gen_args = PackedCausalMambaGeneratorArgs(
    ...
    generator = PackedCausalMambaGenerator(gen_args, model, tokenizer)
...
def main():
    parser = argparse.ArgumentParser(description="Pass@128 evaluation for Mamba on GSM-Infinity")
