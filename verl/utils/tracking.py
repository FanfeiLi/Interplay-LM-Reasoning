...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            proxy = trainer_cfg.get("wandb_proxy") if trainer_cfg else None
            if proxy:
                settings_kwargs["https_proxy"] = proxy

            timeout_val = None
            if trainer_cfg and "wandb_init_timeout" in trainer_cfg:
                timeout_val = trainer_cfg.get("wandb_init_timeout")
            if timeout_val is None:
                env_timeout = os.environ.get("WANDB_INIT_TIMEOUT")
                if env_timeout is not None:
                    timeout_val = env_timeout
            if timeout_val is None:
                timeout_val = 300  # give wandb clients more time to establish the run
            ...
            try:
                ...
                if timeout_float > 0:
                    settings_kwargs["init_timeout"] = timeout_float
