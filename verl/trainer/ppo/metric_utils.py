...
def compute_pass_at_k(successes: int, total: int, k: int) -> float:
    """Compute pass@k probability from success counts."""
        ...
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                ...
                if n_resps > 1:
                    ...
                    ns = [1]  # Start from 1 so pass@1 is always reported
                    n = 2
                    while n < n_resps:
                        ns.append(n)
                        n *= 2
                    ...
                    successes = 0
                    for val in var_vals:
                        if isinstance(val, (int, np.integer, float, np.floating, bool)):
                            fv = float(val)
                            if not math.isfinite(fv):
                                is_binary = False
                                break
                            rounded = int(round(fv))
                            if abs(fv - rounded) > 1e-6 or rounded not in (0, 1):
                                is_binary = False
                                break
                            successes += rounded
                        else:
                    ...
                    for n in ns:
                        ...
                        # )
                        # metric[f"mean@{n}/mean"], metric[f"mean@{n}/std"] = mon_mean, mon_std
                        # metric[f"best@{n}/mean"], metric[f"best@{n}/std"] = bon_mean, bon_std
                        # metric[f"worst@{n}/mean"], metric[f"worst@{n}/std"] = won_mean, won_std
                        if is_binary:
                            metric[f"pass@{min(n, n_resps)}"] = compute_pass_at_k(successes, n_resps, min(n, n_resps))
