#!/bin/bash
# =============================================================================
# NODE B — Run on second 8-GPU node
#
#   Training (~10h):
#     1. grpo_rup_hard_v2                                       ~5h
#     2. grpo_rup_mixed_v2                                      ~5h
#
#   Evals (~2.5h):
#     3. eval grpo_mixed_v2
#     4. eval grpo_rup_hard_v2
#     5. eval grpo_rup_mixed_v2
#     6. eval grpo_rup_strong_hard_v2
#
#   Estimated total: ~12.5h
#
# Usage:
...
for ((i=0; i<${#TRAIN_CONFIGS[@]}; i++)); do
    ...
    echo "[B-$JOB/$TOTAL] ${CFG} done."
done

# ─────────────────────────────────────────────────────────────────────────────
# Evals
# ─────────────────────────────────────────────────────────────────────────────
EVAL_RUNS=("grpo_mixed_v2" "grpo_rup_hard_v2" "grpo_rup_mixed_v2" "grpo_rup_strong_hard_v2")
EVAL_CFGS=("grpo_mixed_v2" "grpo_rup_hard_v2" "grpo_rup_mixed_v2" "grpo_rup_strong_hard_v2")
...
for ((i=0; i<${#EVAL_RUNS[@]}; i++)); do
    ...
    if [ -z "$LATEST" ]; then
        echo "[B-$JOB/$TOTAL] ERROR: No checkpoint for $RUN, skipping."
        continue
    fi
    ...
    echo ""
    echo "================================================================"
    echo "[B-$JOB/$TOTAL] Eval: ${RUN} — global_step_${LATEST}"
    echo "================================================================"
    ...
    echo "[B-$JOB/$TOTAL] ${RUN} eval done."
done
