#!/bin/bash
# =============================================================================
# Evaluate final checkpoint of each v2 run with process-verified pass@128
#
# Uses verl/reward_fn.py:compute_score_with_step_process with
# zero_on_process_mismatch=true (paper's "1.0 Outcome if Process is Correct")
#
# Usage:
#   bash scripts/gsm_infinity_rl/eval_final_v2_process.sh [run1 run2 ...]
#
# If no runs specified, evaluates all GRPO v2 and RUP v2 runs.
# =============================================================================
...
if [ $# -gt 0 ]; then
    ...
    RUNS=(
        ...
        "grpo_rup_strong_id_v2" "grpo_rup_strong_edge_v2" "grpo_rup_strong_hard_v2" "grpo_rup_strong_mixed_v2"
    )
fi
